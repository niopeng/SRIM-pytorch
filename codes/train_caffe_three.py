...
def validate(val_loader, opt, model, current_step, epoch, logger):
    ...
    for val_data in val_loader:
        ...
        if 'zero_code' in opt['train'] and opt['train']['zero_code']:
            ...
            code_val_2 = torch.zeros(val_data['LR'].shape[0], int(opt['network_G']['in_code_nc']),
                                     val_data['LR'].shape[2] * 8, val_data['LR'].shape[3] * 8)
            ...
            code_val_1 = torch.rand(val_data['LR'].shape[0], int(opt['network_G']['in_code_nc']),
                                    val_data['LR'].shape[2] * 4, val_data['LR'].shape[3] * 4)
            code_val_2 = torch.rand(val_data['LR'].shape[0], int(opt['network_G']['in_code_nc']),
                                    val_data['LR'].shape[2] * 8, val_data['LR'].shape[3] * 8)
        else:
            ...
            code_val_1 = torch.randn(val_data['LR'].shape[0], int(opt['network_G']['in_code_nc']),
                                     val_data['LR'].shape[2] * 4, val_data['LR'].shape[3] * 4)
            code_val_2 = torch.randn(val_data['LR'].shape[0], int(opt['network_G']['in_code_nc']),
                                     val_data['LR'].shape[2] * 8, val_data['LR'].shape[3] * 8)
        model.feed_data(val_data, code=[code_val_0, code_val_1, code_val_2])
        model.test()
        ...
        save_img_path = os.path.join(img_dir, 'caffe_{:s}_x8_{:d}.png'.format(img_name, current_step))
...
def main():
    ...
    for epoch in range(num_month):
        for i, train_data in enumerate(train_loader):
            cur_month_code = get_code_for_data_three(model, train_data, opt)
