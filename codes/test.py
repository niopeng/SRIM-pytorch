...
for test_loader in test_loaders:
    ...
    for data in test_loader:
        ...
        for run_index in range(multiple):
            ...
            if 'zero_code' in opt and opt['zero_code']:
                code_val = torch.zeros(data['LR'].shape[0], int(opt['network_G']['in_code_nc']),
                                       data['LR'].shape[2], data['LR'].shape[3])
            elif 'rand_code' in opt and opt['rand_code']:
                code_val = torch.rand(data['LR'].shape[0], int(opt['network_G']['in_code_nc']),
                                      data['LR'].shape[2], data['LR'].shape[3])
            else:
                code_val = torch.randn(data['LR'].shape[0], int(opt['network_G']['in_code_nc']),
                                       data['LR'].shape[2], data['LR'].shape[3])

            model.feed_data(data, code=code_val, need_HR=need_HR)
            ...
            suffix = opt['suffix']
            if suffix:
                save_img_path = os.path.join(dataset_dir, img_name + suffix + '_%d.png' % run_index)
            else:
